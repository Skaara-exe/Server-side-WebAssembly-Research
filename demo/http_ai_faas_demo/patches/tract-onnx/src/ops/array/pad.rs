    ...
    fn wire(
        ...
        let mode = if let Some(constant) = self.constant_input {
            if constant < inputs.len() {
                ...
                // The constant_value input was constant-folded away before wire() is called;
                // fall back to the mode already stored (which holds the folded constant).
                self.mode.clone()
            }
        } else {
            self.mode.clone()
        ...
        let pads = model
            ...
            .cast_to::<i64>()?;
        let pads_slice = pads.as_slice::<i64>()?;
        let input_rank = model.outlet_fact(inputs[0])?.rank();
        let required_len = 2 * input_rank;
        let pads_vec: Vec<i64> = if pads_slice.len() >= required_len {
            pads_slice.to_vec()
        } else {
            let mut v = pads_slice.to_vec();
            v.resize(required_len, 0);
            v
        };
        let pads_slice = &pads_vec[..];
        let rank = (pads_slice.len() / 2).min(input_rank);
        let pads: Vec<(usize, usize)> = (0..input_rank)
            .map(|ax| {
                let left = pads_slice.get(ax).copied().unwrap_or(0) as usize;
                let right = pads_slice.get(ax + rank).copied().unwrap_or(0) as usize;
                (left, right)
            })
            .collect();
